...
| **Database migrations** | Room version 3 with explicit `Migration(2, 3)` for the `alarm_overrides` table. `fallbackToDestructiveMigration()` is still present as a safety net but will not trigger as long as migrations exist. Schema export enabled for future migration tooling. v1.1 added `leave_type` column to `leave_balance` table and changed the unique index from `(year, user_id)` to `(year, user_id, leave_type)`. v3 added the `alarm_overrides` table with unique index `(date, user_id)`. |
